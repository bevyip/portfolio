export const caseStudyNavConfig = {
  confido: {
    accentColor: "#695b4d",
    sections: [
      { id: "confido-context", label: "Context" },
      { id: "confido-problem", label: "Problem" },
      { id: "confido-research", label: "Research" },
      { id: "confido-design-approach", label: "Design Approach" },
      { id: "solution-section", label: "Solution" },
      { id: "confido-reflection", label: "Reflection" },
      { id: "confido-see-next", label: "See Next" },
    ],
  },
  venmo: {
    accentColor: "#008cff",
    sections: [
      { id: "venmo-context", label: "Context" },
      { id: "venmo-research", label: "Research" },
      { id: "solution-section", label: "Solution" },
      { id: "venmo-next-steps", label: "Next Steps" },
      { id: "venmo-takeaways", label: "Takeaways" },
      { id: "venmo-see-next", label: "See Next" },
    ],
  },
  moodle: {
    accentColor: "#49594b",
    sections: [
      { id: "moodle-context", label: "Context" },
      { id: "moodle-research", label: "Research" },
      { id: "solution-section", label: "Solution" },
      { id: "moodle-validation", label: "Validation" },
      { id: "moodle-next-steps", label: "Next Steps" },
      { id: "moodle-takeaways", label: "Takeaways" },
      { id: "moodle-see-next", label: "See Next" },
    ],
  },
  dandi: {
    accentColor: "#e86fa5",
    sections: [
      { id: "dandi-context", label: "Context" },
      { id: "dandi-where-we-began", label: "Where We Began" },
      { id: "dandi-what-we-found", label: "What We Found" },
      { id: "solution-section", label: "Solution" },
      { id: "dandi-next-steps", label: "Next Steps" },
      { id: "dandi-takeaways", label: "Takeaways" },
      { id: "dandi-learn-more", label: "Learn More" },
      { id: "dandi-see-next", label: "See Next" },
    ],
  },
  blockparty: {
    accentColor: "#0f0f0f",
    sections: [
      { id: "blockparty-why", label: "Context" },
      { id: "blockparty-inspiration", label: "Inspiration" },
      { id: "blockparty-problem-making", label: "Design Iterations" },
      { id: "blockparty-full-experience", label: "Demo Experience" },
      { id: "blockparty-whats-next", label: "What's Next" },
      { id: "blockparty-learning-points", label: "Takeaways" },
      { id: "blockparty-closing", label: "Closing" },
    ],
  },
};
